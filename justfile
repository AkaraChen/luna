...
test:
    ...
    bun run --cwd '{{ justfile_directory() / "apps/asahi-web" }}' test
...
asahi-backend: _ensure-cargo-watch
    ASAHI_PORT='{{ asahi_dev_port }}' ASAHI_DATABASE_URL='{{ asahi_dev_database_url }}' DATABASE_URL='{{ asahi_dev_database_url }}' ROCKET_ADDRESS=127.0.0.1 ROCKET_PORT='{{ asahi_dev_port }}' ASAHI_SKIP_WEB_BUILD=1 cargo watch -w crates/asahi -w Cargo.toml -w Cargo.lock -x 'run -p asahi'
